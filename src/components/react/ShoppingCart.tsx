...
const ShoppingCart = () => {
  ...
  return (
    <section className="fixed bottom-0 w-full flex flex-col items-center justify-center h-10">
      <div
        className={`absolute bg-primary w-1/2 rounded-t shadow-3xl-inner grid grid-cols-2 bottom-0 ${isCartOpen.value ? "-translate-y-15" : "translate-y-[21.5rem]"}  transform delay-75 duration-500 ease-in-out`}
      >
        <section className="flex justify-around items-center col-span-2 my-3 h-30 w-full">
          ...
          <div className="flex items-center justify-around flex-1">
            <a
              ...
              className={`px-5 py-2 bg-secondary text-3xl text-primary rounded shadow-3xl hover:bg-[#B3B770] ${isCartOpen.value ? "opacity-0 pointer-events-none" : "opacity-100 pointer-events-auto"} transform delay-75 duration-500 ease-in-out`}
            >
              Pagar
            </a>
        ...
        <div className="w-full col-span-2 py-4 flex justify-end mt-1">
          <a
            href="/pedido/checkout"
            className={`px-5 py-2 mr-3 bg-secondary text-3xl text-primary rounded shadow-3xl hover:bg-[#B3B770] ${!isCartOpen.value ? "opacity-0 pointer-events-none" : "opacity-100 pointer-events-auto"}`}
          >
            Pagar
          </a>
        </div>
